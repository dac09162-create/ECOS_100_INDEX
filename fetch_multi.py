...
def fetch(api_key, stat_code, i1, i2, cycle):
    ...
    url = (f"https://ecos.bok.or.kr/api/StatisticSearch/"
           ...
           f"{stat_code}/{cycle}/{start}/{end}/{i1}/{i2}")
    try:
        ...
        # ✅ i1, i2 정확히 필터링
        if "ITEMCODE1" in df.columns and i1:
            df = df[df["ITEMCODE1"] == i1]
        if "ITEMCODE2" in df.columns and i2:
            df = df[df["ITEMCODE2"] == i2]
...
def init_db(con):
    con.execute("""
        CREATE TABLE IF NOT EXISTS timeseries_multi (
            sector VARCHAR, name VARCHAR, stat_code VARCHAR,
            item_code1 VARCHAR, item_code2 VARCHAR, cycle VARCHAR,
            time VARCHAR, value DOUBLE, updated_at DATE
        )
    """)

def upsert(con, sector, name, stat_code, i1, i2, cycle, df):
    if df.empty: return 0
    df = df.copy()
    df["sector"] = sector; df["name"] = name
    df["stat_code"] = stat_code; df["item_code1"] = i1; df["item_code2"] = i2
    df["cycle"] = cycle; df["updated_at"] = date.today().isoformat()
    df = df.rename(columns={"TIME": "time"})
    df = df[["sector","name","stat_code","item_code1","item_code2","cycle","time","value","updated_at"]]
    con.execute("DELETE FROM timeseries_multi WHERE stat_code=? AND item_code1=? AND item_code2=? AND name=? AND cycle=?",
                [stat_code, i1, i2, name, cycle])
    con.execute("INSERT INTO timeseries_multi SELECT * FROM df")
    return len(df)

def main():
    with open("indicators_config_multi.json", encoding="utf-8") as f:
        config = json.load(f)
